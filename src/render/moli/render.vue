	...
	for (let i = 0; i < horizontalBlockCount; i++) {
		...
		blocks.push(
			...[
				{
					...commonBlock,
					x: BLOCK_PADDING + i * blockSize.value,
					y: isReverse ? BLOCK_PADDING : BLOCK_PADDING + blockSize.value,
				},
				{
					...commonBlock,
					x: BLOCK_PADDING + i * blockSize.value,
					y: isReverse
						? props.height - BLOCK_PADDING - blockSize.value
						: props.height - BLOCK_PADDING - blockSize.value * 2,
				},
			],
		);
	}
	...
	for (let i = 0; i < verticalBlockCount; i++) {
		...
		blocks.push(
			...[
				{
					...commonBlock,
					x: isReverse ? BLOCK_PADDING : BLOCK_PADDING + blockSize.value,
					y: BLOCK_PADDING + blockSize.value * i,
				},
				{
					...commonBlock,
					x: isReverse
						? props.width - BLOCK_PADDING - blockSize.value
						: props.width - BLOCK_PADDING - blockSize.value * 2,
					y: BLOCK_PADDING + blockSize.value * i,
				},
			],
		);
