	...
	for (let i = 0; i < horizontalBlockCount; i++) {
		blocks.push(
			...[
				...
				{
					...commonBlock,
					x: 109 + (BLOCK_SIZE + BLOCK_PADDING) * i,
					y: 21 + BLOCK_SIZE,
					image: blackWheatearState.value,
				},
				{
					...commonBlock,
					x: 109 + (BLOCK_SIZE + BLOCK_PADDING) * i,
					y: props.height - RECT_SIZE - BLOCK_SIZE,
					image: blackWheatearState.value,
				},
		...
		blocks.push(
			...[
				{ ...verticalCommonBlock, x: 21, y: 109 + BLOCK_SIZE + (BLOCK_SIZE + BLOCK_PADDING) * i },
				{
					...verticalCommonBlock,
					x: RECT_SIZE,
					y: 109 + BLOCK_SIZE + (BLOCK_SIZE + BLOCK_PADDING) * i,
					image: blackWheatearState.value,
				},
				{
					...verticalCommonBlock,
					x: props.width - (RECT_SIZE + BLOCK_SIZE),
					y: 109 + BLOCK_SIZE + (BLOCK_SIZE + BLOCK_PADDING) * i,
					image: blackWheatearState.value,
				},
